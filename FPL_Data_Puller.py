...
def FPL_DATA_PULL():
    ...
    chips_count_df = chips_count_df.rename(columns={'player_name':'player_name', 'chips':'chip', 'name':'count'}).reset_index()

    # create chips used by chip
    chips_count_by_chip = chips_used_df[['player_name', 'chips', 'name']].groupby(by=['player_name', 'chips']).count()
    chips_count_by_chip = chips_count_by_chip.rename(columns={'player_name':'player_name', 'chips':'chip', 'name':'count'}).reset_index()
    ...
    manager_data = manager_data.assign(actual_bank = lambda x: x.bank/10)
    manager_data = manager_data.assign(actual_value = lambda x: x.value/10)
    manager_data.rename(columns={'event':'gameweek'}, inplace=True)
    ...
    league_standings['points before transfer cost']= league_standings['total']+league_standings['event_transfers_cost']

    return manager_data, average_entry_score_df, league_standings, league_ids, chips_count_df, chips_count_by_chip